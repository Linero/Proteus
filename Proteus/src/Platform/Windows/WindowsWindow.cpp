...
namespace Proteus {
	...
	static void GLFWErrorCallback(int error, const char* description) {
		...
		PROTEUS_CORE_ERROR("GLFW Error ({0}): {1}", error, description);
	}
	...
	void WindowsWindow::Init(const WindowProps& props)
	{
		...
		PROTEUS_CORE_INFO("Creating window {0} [{1},{2}]", props.Title, props.Width, props.Height);

		if (!s_GLFWInitialized) {
			int success = glfwInit();
			PROTEUS_CORE_ASSERT(success, "Could not initialize GLFW");
